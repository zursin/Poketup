...
// Replace spawning block using indexOf
const spawnStart = content.indexOf('          let enemySpecies = globalScene.randomSpecies(battle.waveIndex, level, true);');
const spawnEndStr = '          );';
const spawnEnd = content.indexOf(spawnEndStr, spawnStart) + spawnEndStr.length;
...
if (spawnStart !== -1 && spawnEnd !== -1) {
  ...
  content = content.substring(0, spawnStart) + spawnNew + content.substring(spawnEnd);
  console.log("Replaced spawn block");
} else {
  console.log("Could not find spawn block");
}

// Replace Wonder Guard
const wgStart = content.indexOf('      const enemyPokemon = globalScene.getEnemyParty()[e];');
const wgEndStr = '      }';
const wgEnd = content.indexOf(wgEndStr, content.indexOf('enemyPokemon.fieldSetup(true);')) + wgEndStr.length;

if (wgStart !== -1 && wgEnd !== -1) {
  const wgNew = `      const enemyPokemon = globalScene.getEnemyParty()[e];
  ...
  content = content.substring(0, wgStart) + wgNew + content.substring(wgEnd);
  console.log("Replaced WG block");
} else {
  console.log("Could not find WG block");
}

// Replace Dialogue block
const diagStart = content.indexOf('        const doTrainerSummon = () => {');
const diagEndStr = '        }';
const diagEnd = content.indexOf(diagEndStr, content.indexOf('          doTrainerSummon();')) + diagEndStr.length;

if (diagStart !== -1 && diagEnd !== -1) {
  const diagNew = `        const doTrainerSummon = () => {
  ...
  content = content.substring(0, diagStart) + diagNew + content.substring(diagEnd);
  console.log("Replaced diag block");
} else {
  console.log("Could not find diag block");
}
...
if (!content.includes('PokemonMove')) {
  content = content.replace('import { Pokemon } from "#field/pokemon";', 'import type { Pokemon } from "#field/pokemon";\nimport { PokemonMove } from "#moves/pokemon-move";');
}
